"""
Download EUR/USD H1 BID data from Dukascopy (2015-01-01 -> 2026-04-18).
Uses npx dukascopy-node in 6-month chunks, then merges into a single CSV.
"""
import subprocess
import sys
import io
...
OUT_DIR = Path("data/download/eurusd_h1")
MERGED_FILE = Path("data/download/eurusd-h1-bid-2015-01-01-2026-04-18.csv")
...
def download_chunk(date_from, date_to):
    cmd = (
        f'npx dukascopy-node -i eurusd '
        f'-from {date_from} -to {date_to} '
        f'-t h1 -p bid -f csv -v --flats '
...
def main():
    ...
    print(f"Downloading EUR/USD H1 BID data to {OUT_DIR}")
