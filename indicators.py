...
def prepare_indicators(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df['BB_upper'] = df['SMA50'].rolling(20).mean() + 2 * _bb_std  # using 20-period SMA
    df['BB_lower'] = df['SMA50'].rolling(20).mean() - 2 * _bb_std
    ...
    # ── Swing High/Low 支撑阻力位 (vectorized) ──
    _swing_n = 10
    _win = 2 * _swing_n + 1
    _roll_max = df['High'].rolling(_win, center=True).max()
    df['swing_high'] = df['High'].where(df['High'] == _roll_max, np.nan)
    _roll_min = df['Low'].rolling(_win, center=True).min()
    df['swing_low'] = df['Low'].where(df['Low'] == _roll_min, np.nan)
    df['nearest_resistance'] = df['swing_high'].ffill()
    df['nearest_support'] = df['swing_low'].ffill()
    df['dist_to_resistance'] = (df['nearest_resistance'] - df['Close']) / df['ATR'].replace(0, np.nan)
    df['dist_to_support'] = (df['Close'] - df['nearest_support']) / df['ATR'].replace(0, np.nan)

    # ── 顶底分型 (Top/Bottom Fractal) ──
    h1, h2, h3 = df['High'].shift(2), df['High'].shift(1), df['High']
    l1, l2, l3 = df['Low'].shift(2), df['Low'].shift(1), df['Low']
    c3 = df['Close']
    o3 = df['Open']
    df['top_fractal'] = ((h2 > h1) & (h2 > h3) & (c3 < o3)).astype(float)
    df['bot_fractal'] = ((l2 < l1) & (l2 < l3) & (c3 > o3)).astype(float)

    # ── 孕线 (Inside Bar) — 第3根K确认方向 ──
    prev_h = df['High'].shift(1)
    prev_l = df['Low'].shift(1)
    pprev_h = df['High'].shift(2)
    pprev_l = df['Low'].shift(2)
    is_inside = (prev_h <= pprev_h) & (prev_l >= pprev_l)
    df['inside_bar_bull'] = (is_inside & (df['Close'] > pprev_h)).astype(float)
    df['inside_bar_bear'] = (is_inside & (df['Close'] < pprev_l)).astype(float)

    # ── 2B / Engulfing ──
    prev_body_top = df[['Close', 'Open']].shift(1).max(axis=1)
    prev_body_bot = df[['Close', 'Open']].shift(1).min(axis=1)
    cur_body_top = df[['Close', 'Open']].max(axis=1)
    cur_body_bot = df[['Close', 'Open']].min(axis=1)
    df['engulf_bull'] = ((df['Close'] > df['Open'])
                         & (df['Open'].shift(1) > df['Close'].shift(1))
                         & (cur_body_top >= prev_body_top)
                         & (cur_body_bot <= prev_body_bot)).astype(float)
    df['engulf_bear'] = ((df['Close'] < df['Open'])
                         & (df['Open'].shift(1) < df['Close'].shift(1))
                         & (cur_body_top >= prev_body_top)
                         & (cur_body_bot <= prev_body_bot)).astype(float)

    # ── 日幅波动 (Daily Range from open) ──
    _date = df.index.date
    _date_series = pd.Series(_date, index=df.index)
    _daily_open = df.groupby(_date_series)['Open'].transform('first')
    df['daily_range_up'] = df['High'] - _daily_open
    df['daily_range_down'] = _daily_open - df['Low']
    df['daily_max_range'] = df[['daily_range_up', 'daily_range_down']].max(axis=1)

    # ── 共振信号计数 (同一根bar上出现的看多/看空K线形态数) ──
