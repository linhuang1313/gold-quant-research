//+------------------------------------------------------------------+
//| L7_ExecutionEdge_EA.mq4                                          |
//| L7(MH=8) + H1 KC Filter + EqCurve + Execution Edge              |
//| 新增: KCBW5 降频 | MaxLoss Cap | 时段过滤                          |
//| Chart: XAUUSD M15                                                 |
//+------------------------------------------------------------------+
...
extern int    MagicNumber      = 20250426;
extern int    MaxSlippage       = 30;

//--- L7 Keltner 入场参数 (信号在 H1 KC 上计算)
extern int    KC_EMA_Period     = 25;
extern double KC_Multiplier     = 1.2;
extern int    ATR_Period        = 14;
extern int    ADX_Period        = 14;
extern double ADX_Threshold     = 18.0;
extern int    EMA100_Period     = 100;

//--- L7 出场参数
extern double SL_ATR_Mult       = 3.5;
extern double TP_ATR_Mult       = 8.0;
extern int    MaxHold_Bars      = 8;
extern double Trail_Act_ATR     = 0.28;
extern double Trail_Dist_ATR    = 0.06;

//--- TATrail
extern int    TATrail_Start     = 2;
extern double TATrail_Decay     = 0.75;
extern double TATrail_Floor     = 0.003;

//--- Regime
extern double Regime_Low_Act    = 0.40;
extern double Regime_Low_Dist   = 0.10;
extern double Regime_Norm_Act   = 0.28;
extern double Regime_Norm_Dist  = 0.06;
extern double Regime_High_Act   = 0.12;
extern double Regime_High_Dist  = 0.02;
...
//--- KCBW5 降频 (KC Bandwidth 5根过滤)
extern bool   KCBW_Enabled      = true;
extern int    KCBW_Lookback     = 5;
extern double KCBW_Min_Ratio    = 0.0;  // 0=自动 (要求BW在扩张中)

//--- MaxLoss Cap (单笔硬封顶)
extern bool   MaxLoss_Enabled   = true;
extern double MaxLoss_USD       = 35.0;  // 0.03手约$35对应~$1167点

//--- 时段过滤 (UTC时间)
extern bool   Session_Filter    = true;
extern int    Session_Skip_Start = 2;    // UTC 02:00 开始跳过
extern int    Session_Skip_End   = 5;    // UTC 05:00 结束跳过
...
int OnInit()
{
   ...
   Print("L7 ExecutionEdge EA v2.0 | Lot=", LotSize,
         " KCBW=", KCBW_Enabled, " MaxLoss=", MaxLoss_USD,
...
bool IsKCBandwidthExpanding()
{
   ...
   // BW must be higher than the minimum of last KCBW_Lookback bars
   double minBW = bw[1];
...
void CheckMaxLossCap()
{
   ...
   if(profit < -MaxLoss_USD)
   {
      ...
      if(closed)
      {
         Print("MaxLoss Cap triggered! Loss=$", DoubleToStr(MathAbs(profit), 2),
...
void OnTick()
{
   // MaxLoss Cap runs on every tick
   CheckMaxLossCap();

   // Main logic on new M15 bar only
   ...
   // 2. Session filter (Execution Edge)
   ...
   // 5. KCBW5 bandwidth filter (Execution Edge)
   if(!IsKCBandwidthExpanding())
   {
      // Print("KCBW filter blocked: bandwidth not expanding");
      return;
   }

   // 6. EqCurve risk control
   if(ShouldSkipByEqCurve())
   {
      Print("EqCurve skip: avg PnL < ", EqCurve_Cut);
      return;
   }
   ...
   if(signal == 1)
   {
      ...
      ticket = OrderSend(Symbol(), OP_BUY, lot, ask_price, MaxSlippage,
                         sl, tp, "L7v2 BUY", MagicNumber, 0, clrBlue);
      if(ticket > 0)
      {
   ...
   else if(signal == -1)
   {
      ...
      ticket = OrderSend(Symbol(), OP_SELL, lot, bid_price, MaxSlippage,
                         sl, tp, "L7v2 SELL", MagicNumber, 0, clrRed);
      if(ticket > 0)
      {
         lastEntryTime = TimeCurrent();
         barsHeld = 0;
