"""Launch R24 on remote and poll until done, then download results."""
...
print("=== Kill old processes ===", flush=True)
c = connect()
run(c, 'pkill -f "run_round24" 2>/dev/null; sleep 1; echo killed')
c.close()

# Launch
print("\n=== Launch R24 ===", flush=True)
c = connect()
...
time.sleep(5)
run(c, 'ps aux | grep "python3.*run_round24" | grep -v grep')
time.sleep(3)
run(c, f"head -30 {DIR}/results/round24_results/stdout.txt 2>/dev/null")
c.close()

# Poll
print("\n=== Monitoring (30s intervals, max 60min) ===", flush=True)
for i in range(120):
    time.sleep(30)
    try:
        c = connect()
        out, _ = run(c, f"tail -10 {DIR}/results/round24_results/stdout.txt 2>/dev/null")
