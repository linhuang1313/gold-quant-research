"""Deploy R80 MACD 8-stage validation to server."""
...
FILES = [
    'experiments/run_r80_macd_validation.py',
...
_, out, _ = c.exec_command("mkdir -p /root/gold-quant-research/results/r80_macd_validation", timeout=15)
out.read()

print("\nLaunching R80...", flush=True)
c.exec_command(
    "cd /root/gold-quant-research && "
    "nohup python3 -u experiments/run_r80_macd_validation.py "
    "> results/r80_stdout.txt 2>&1 &",
    timeout=10
)
time.sleep(5)
_, out, _ = c.exec_command("ps aux | grep r80 | grep -v grep", timeout=15)
try:
