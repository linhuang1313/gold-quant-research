"""Deploy Round 16 (Timeout Sniper) to Server C and start."""
import paramiko
import sys
import os
import time
...
FILES_TO_UPLOAD = [
    ...
    ("experiments/run_round16.py", "scripts/experiments/run_round16.py"),
]
...
def main():
    print("=" * 60)
    print("Deploy Round 16 — Timeout Sniper -> Server C")
    print("=" * 60)
    ...
    if out:
        ...
        time.sleep(5)
        ssh_exec(client, "pkill -f 'run_round1[3-6]' 2>/dev/null")
        time.sleep(2)
    ...
    # Create dirs
    ssh_exec(client, f"mkdir -p {REMOTE_DIR}/logs {REMOTE_DIR}/results/round16_results")
    ...
    # Start R16
    cmd = (f"cd {REMOTE_DIR} && nohup python3 -u scripts/experiments/run_round16.py "
           f"> logs/round16.log 2>&1 &")
    print(f"\n--- Starting R16 ---")
    client.exec_command(cmd)
    time.sleep(5)

    out, _ = ssh_exec(client, "ps aux | grep run_round16 | grep -v grep")
    if out:
        print("  R16 started successfully!")
        for line in out.split("\n"):
            parts = line.split()
            if len(parts) > 10:
                print(f"    PID={parts[1]} CPU={parts[2]}% CMD={' '.join(parts[10:])}")
    else:
        print("  WARNING: R16 may not have started. Checking logs...")
        out, _ = ssh_exec(client, f"tail -30 {REMOTE_DIR}/logs/round16.log 2>/dev/null",
                         timeout=30)
        print(out)

    # Check initial log
    time.sleep(3)
    out, _ = ssh_exec(client, f"tail -10 {REMOTE_DIR}/logs/round16.log 2>/dev/null")
    if out:
        print(f"\n--- Initial log ---")
        print(out)
    ...
    client.close()
    print(f"\n{'='*60}")
    print("Deploy complete! Monitor with: python deploy/_check_r16.py")
    print(f"{'='*60}")
