"""Deploy R112 Gold/Silver pair trading to server (script + data files)."""
...
def main():
    print("=" * 50)
    print("  Deploying R112 Gold/Silver Pair Trading")
    print("=" * 50)
    ...
    for f in data_files:
        local = os.path.join(LOCAL_BASE, 'data', f)
        remote = f"{REMOTE_BASE}/data/{f}"
    ...
    if pid:
        ...
        print(f"  R112 NOT RUNNING! Checking output...")
        c = conn()
        _, out, _ = c.exec_command(
