"""Deploy R96-R100 experiments to remote server and run in parallel.
...
def main():
    print("=" * 60)
    print("Deploying R96 + R97 + R98 + R99 + R100 (parallel on remote)")
    print("=" * 60)

    experiments = [
        ('run_r96_slippage.py', 'r96_slippage', 'r96_stdout.txt'),
        ('run_r97_correlation.py', 'r97_correlation', 'r97_stdout.txt'),
        ('run_r98_ml_threshold.py', 'r98_ml_threshold', 'r98_stdout.txt'),
        ('run_r99_time_effects.py', 'r99_time_effects', 'r99_stdout.txt'),
        ('run_r100_circuit_breaker.py', 'r100_circuit_breaker', 'r100_stdout.txt'),
    ]
    ...
    c = connect()
    _, out, _ = c.exec_command("ps aux | grep -E 'r9[6-9]|r100' | grep -v grep", timeout=15)
    ps = out.read().decode()
    c.close()
    ...
    print("\n" + "=" * 60)
    print(f"Deploy complete! {len(running)}/5 experiments running.")
    print("Monitor with: tail -f results/<dir>/<stdout>.txt on the server")
