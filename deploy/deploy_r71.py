"""Deploy R71 OOS validation to server."""
...
FILES = [
    'experiments/run_r71_oos_validation.py',
...
_, out, _ = c.exec_command("mkdir -p /root/gold-quant-research/results/r71_oos", timeout=15)
out.read()
print("Dirs created.")

c.exec_command(
    "cd /root/gold-quant-research && "
    "nohup python3 -u experiments/run_r71_oos_validation.py "
    "> results/r71_stdout.txt 2>&1 &",
    timeout=10
)
time.sleep(5)

_, out, _ = c.exec_command("ps aux | grep r71 | grep -v grep", timeout=15)
try:
