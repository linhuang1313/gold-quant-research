"""Deploy Round 15 (TATrail Deep Validation) to Server C and start."""
import paramiko
import sys
import os
import time
...
FILES_TO_UPLOAD = [
    ...
    ("experiments/run_round15.py", "scripts/experiments/run_round15.py"),
]
...
def main():
    print("=" * 60)
    print("Deploy Round 15 — TATrail Deep Validation → Server C")
    print("=" * 60)

    client = connect()

    # Check for running experiments
    out, _ = ssh_exec(client, "ps aux | grep -E 'run_round|run_exp' | grep python | grep -v grep")
    if out:
        ...
        time.sleep(5)
        ssh_exec(client, "pkill -f run_round15 2>/dev/null")
        time.sleep(2)
    ...
    # Create dirs
    ssh_exec(client, f"mkdir -p {REMOTE_DIR}/logs {REMOTE_DIR}/results/round15_results")
    ...
    # Start R15
    cmd = (f"cd {REMOTE_DIR} && nohup python3 -u scripts/experiments/run_round15.py "
           f"> logs/round15.log 2>&1 &")
    print(f"\n--- Starting R15 ---")
    client.exec_command(cmd)
    time.sleep(5)

    out, _ = ssh_exec(client, "ps aux | grep run_round15 | grep -v grep")
    if out:
        print("  R15 started successfully!")
        for line in out.split("\n"):
            parts = line.split()
            if len(parts) > 10:
                print(f"    PID={parts[1]} CPU={parts[2]}% CMD={' '.join(parts[10:])}")
    else:
        print("  WARNING: R15 may not have started. Checking logs...")
        out, _ = ssh_exec(client, f"tail -30 {REMOTE_DIR}/logs/round15.log 2>/dev/null",
                         timeout=30)
        print(out)

    # Check initial log
    time.sleep(3)
    out, _ = ssh_exec(client, f"tail -10 {REMOTE_DIR}/logs/round15.log 2>/dev/null")
    if out:
        print(f"\n--- Initial log ---")
        print(out)
    ...
    client.close()
    print(f"\n{'='*60}")
    print("Deploy complete! Monitor with: python deploy/_check_r15.py")
    print(f"{'='*60}")
