#!/usr/bin/env python3
"""Download XAUUSD H4 Bid data from Dukascopy (2015-01-01 -> today)."""
import sys
import time
...
OUT_FILE = DL_DIR / f"xauusd-h4-bid-2015-01-01-{TODAY_STR}.csv"
...
def main():
    if OUT_FILE.exists():
        ...
        print(f"H4 file already exists: {OUT_FILE.name} ({len(existing)} rows)")
        print("Delete it to re-download.")
        return

    print(f"Downloading XAUUSD H4 BID: {START.date()} -> {END.date()}")
    t0 = time.time()
